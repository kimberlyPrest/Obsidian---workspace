...
const TEXT_MSG_TYPES = new Set(['conversation', 'extendedTextMessage'])
...
async function fetchEvolutionMessages(opts: {
  ...
  const records = (data?.messages?.records ?? data?.records ?? data?.data) as Json[] | undefined
  return records ?? []
}

async function upsertContact(opts: { instanceId: string; remoteJid: string; pushName?: string }) {
  const { data, error } = await sb
  ...
  try {
    ...
    for (const m of textMessages) {
      try {
        ...
        const { error: upErr } = await sb.from('whatsapp_messages').upsert(
          {
            instance_id: instance.id,
